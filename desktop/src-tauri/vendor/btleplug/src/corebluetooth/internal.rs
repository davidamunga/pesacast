    ...
    fn check_discovered(&mut self) {
        ...
        if !self.services.values().any(|service| !service.discovered) {
            if self.connected_future_state.is_none() {
                // CoreBluetooth can fire service/characteristic discovery delegates more than
                // once for the same peripheral. If the connection future was already resolved
                // on a prior call, simply ignore the duplicate notification.
                return;
            }
            let services = self
                .services
